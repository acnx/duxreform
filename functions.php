...
function showThumb($obj,$size=null,$link=false){
    ...
    if(empty($thumb) && empty($options->default_thumb)){
        $thumb= $options->themeUrl .'/img/thumb/' . rand(1, 25) . '.jpg';
        //去掉下面4行双斜杠 启用BING美图随机缩略图
        //$str = file_get_contents('http://cn.bing.com/HPImageArchive.aspx?format=js&idx='.rand(1, 26).'&n=1');
...
function hotpost() {
    ...
    $defaults = array(
        ...
        'before' => '',
        'after' => '',
        'xformat' => '<article class="excerpt-minic excerpt-minic-index"><h2><span class="red">【置顶】</span><a href="{permalink}" title="{title}">{title}</a></h2></p><p class="note">{content}...</article>'
    );
    $db = Typecho_Db::get();
    ...
    foreach($result as $val){
        ...
        echo str_replace(array('{permalink}', '{title}','{content}'),array($val['permalink'], $val['title'],substr($val['text'],0,208)), $defaults['xformat']);
    }
    echo $defaults['after'];
}
...
function slout() {
    ...
    if (!empty($options->slidercode)) {
        ...
        $text='<a target="_blank" href="#"><img src="'. $options->themeUrl .'/img/banner.jpg"></a>
               <a target="_blank" href="#"><img src="'. $options->themeUrl .'/img/banner.jpg"></a>';
    }
    $t_arr = explode('
', $text);
...
function sitebar_ad($obj) {
    ...
    if (!empty($options)) {
        ...
        $text="https://github.com/Typecho-theme-DUX\nDUX主题 新一代主题\nDUX for Typecho";
    }
    $b_arr = explode("\n", $text);
    return $b_arr;
}
...
// 在下面按格式输入本站创建的时间
    $site_create_time = strtotime('2020-09-30 00:00:00');
    ...
    if(is_numeric($time)){
        ...
        //echo '<span class="btime">'.$value['years'].'年'.$value['days'].'天'.$value['hours'].'小时</span>';
        echo '<span class="btime">'.$value['days'].'天'.$value['hours'].'小时</span>';
    }else{
    echo '';
    }
}
